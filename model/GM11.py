...
def connect_database(address, flag):
    ...
    cls = conn1.cursor()
    if flag == 0:
        sql = "select year,month,oprice from househistory where address = %s order by year DESC, month DESC limit 12"
    else:
        sql = "select year,month,nprice from househistory where address = %s  order by year DESC, month DESC limit 12"
